(let [snap-io (require :snap.common.io)]
  (fn [full-path path bufnr]
    ;; Sets max size for syntax highlighting
    (local max-size (* 1024 10))

    ;; We want to disable TS when the size is too large
    ;; as nvim chokes on large files, e.g. minified JS files
    (when
      (< (snap-io.size full-path) max-size)
      (do
